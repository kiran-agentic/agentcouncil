...
> Runs the AgentCouncil **configure** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
> **Arguments:** [what to configure, e.g. "use gpt-5 via cursor" or "add openrouter as default"]
