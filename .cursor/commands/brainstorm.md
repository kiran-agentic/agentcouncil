...
> Runs the AgentCouncil **brainstorm** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
