Council-governed autonomous delivery. The host agent follows workflow recipes to plan and build; independent agents review at every stage transition. The full pipeline — spec, plan, build, verify, ship — with review loops and conditional challenge gates.

> Runs the AgentCouncil **autopilot** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
> **Arguments:** [what to build — describe the feature, fix, or change] [backend=<review-profile>] [challenge_backend=<challenge-profile>] [review_depth=<legacy|fast|balanced|deep>] [lead_review_model=<model>]
