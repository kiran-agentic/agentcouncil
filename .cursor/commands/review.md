Run the AgentCouncil review protocol. You (the host agent) frame the review question and direct the outside agent to the right files. The outside agent reviews independently. Use when work is done and you want independent critique. Supports --loop for iterative convergence (fix → re-review → verify until clean).

> Runs the AgentCouncil **review** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
> **Arguments:** [what to review — plan, implementation, milestone, design] [--loop to iterate until clean]
