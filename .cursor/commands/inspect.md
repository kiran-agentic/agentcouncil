Inspect past AgentCouncil deliberation sessions. View transcripts, findings, provenance, and protocol history from the journal. Use when you want to look back at a past brainstorm, review, decide, or challenge session.

> Runs the AgentCouncil **inspect** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
> **Arguments:** [session_id or "recent" to list recent sessions]
