...
> Runs the AgentCouncil **challenge** protocol through the `agentcouncil` MCP server. Configure it in `.cursor/mcp.json` (see `docs/CURSOR.md`); the `mcp__agentcouncil__*` tools referenced below are exposed by that server.
>
> **Arguments:** [plan or design to stress-test]
